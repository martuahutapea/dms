...
if (!isset($_SESSION["user_id"]) || !isset($_SESSION["password_hash"]) || $_SESSION["user_role"] != "student") {
...
if (isset($_POST['update_password'])) {
    if (isset($_POST['old_password'], $_POST['new_password'], $_POST['confirm_password'])) {
        ...
        if ($connection) {
            $stmt = $connection->prepare("SELECT student_password FROM student WHERE ps_id = ?");
            ...
            if ($result->num_rows > 0) {
                ...
                if (password_verify($old_password, $hashed_password)) {
                    if ($new_password === $confirm_password) {
                        ...
                        $stmt = $connection->prepare("UPDATE student SET student_password = ? WHERE ps_id = ?");
